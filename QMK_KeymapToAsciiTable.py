...
for keymap in keymaps:
    ...
    for i in range(len(keymap)):
        ...
        for item in keymap[i+1]:
            print(keycodes[item+','][:-1]+'│', end='',file=file) 
        ...
        if len(keymap[i+2]) > len(keymap[i+1]):
            for _ in range(len(keymap[i+2]) - len(keymap[i+1])):
                if not i == 0 and j == len(keymap[i])-7:
                    print("──────┐", end='',file=file)
                else:
                    print(Btop, end='',file=file)
